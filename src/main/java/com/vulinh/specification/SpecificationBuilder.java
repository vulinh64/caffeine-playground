...
/** E stands for 'Entity' F stands for 'Field' */
...
public class SpecificationBuilder {
  ...
  public static <E, F> Specification<E> eq(
      SingularAttribute<? super E, ? extends F> attribute, F value) {
    return (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get(attribute), value);
  }
  ...
  public static <E, F> Specification<E> in(
      SingularAttribute<? super E, ? extends F> attribute, Collection<? extends F> values) {
    return (root, query, criteriaBuilder) -> root.get(attribute).in(values);
  }
  ...
  public static <E, F> Specification<E> ni(
      SingularAttribute<? super E, ? extends F> attribute, Collection<? extends F> values) {
    return Specification.not(in(attribute, values));
  }
  ...
  public static <E> Specification<E> like(String attribute, String value) {
    return (root, query, criteriaBuilder) ->
        criteriaBuilder.like(root.get(attribute), value, BACKSLASH);
  }

  @SuppressWarnings("unchecked")
  public static <E, F> Specification<E> like(
      SingularAttribute<? super E, ? extends F> attribute, String value) {
    if (String.class.isAssignableFrom(attribute.getJavaType())) {
      return (root, query, criteriaBuilder) ->
          criteriaBuilder.like(
              root.get((SingularAttribute<? super E, String>) attribute), value, '\\');
    }

    return (root, query, criteriaBuilder) ->
        criteriaBuilder.like(root.get(attribute).as(String.class), value, '\\');
  }
  ...
  public static <E, F extends Comparable<? super F>> Specification<E> between(
      String attribute, F fromValue, F toValue) {
    if (fromValue.compareTo(toValue) > 0) {
      throwBecauseInvalidBounds(fromValue, toValue);
    }

    return (root, query, criteriaBuilder) ->
        criteriaBuilder.between(root.get(attribute), fromValue, toValue);
  }

  public static <E, F extends Comparable<? super F>> Specification<E> between(
      SingularAttribute<? super E, ? extends F> attribute, F fromValue, F toValue) {
    if (fromValue.compareTo(toValue) > 0) {
      throwBecauseInvalidBounds(fromValue, toValue);
    }
  ...
  private static <F extends Comparable<? super F>> void throwBecauseInvalidBounds(
      F fromValue, F toValue) {
    throw new IllegalArgumentException(
        String.format("fromValue [%s] is greater than toValue [%s]", fromValue, toValue));
  }
}
